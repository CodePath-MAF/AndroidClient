...
public class CircularProgressBar extends ProgressBar {
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(mTitle)) {
            ...
            String[] subtitles = mSubTitle.split("\n");

            for (int i = 0; i < subtitles.length; i++) {
                yPos += titleHeight;
                xPos = (int) (getMeasuredWidth() / 2 - mSubtitlePaint.measureText(subtitles[i]) / 2);

                canvas.drawText(subtitles[i], xPos, yPos, mSubtitlePaint);
            }
        }
    ...
    public void animateProgressTo(final int start, final int end,
        ...
        final ObjectAnimator progressBarAnimator = ObjectAnimator.ofFloat(this, "animateProgress",
                start, end);
